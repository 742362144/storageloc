...
use crate::cmd::{Get, Publish, Set, Subscribe, Unsubscribe, Invoke};
...
impl Client {
    ...
    #[instrument(skip(self))]
    pub async fn invoke(&mut self, key: &str, value: Bytes) -> crate::Result<()> {
        // Create a `Set` command and pass it to `set_cmd`. A separate method is
        // used to set a value with an expiration. The common parts of both
        // functions are implemented by `set_cmd`.
        self.invoke_cmd(Invoke::new(key, value, None)).await
    }
    ...
    async fn invoke_cmd(&mut self, cmd: Invoke) -> crate::Result<()> {
        // Convert the `Set` command into a frame
        let frame = cmd.into_frame();
